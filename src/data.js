...
function isStorageSetup(defaultProjectName, rawProjectList) {
  if (
    rawProjectList === null ||
    !Object.keys(rawProjectList).includes(defaultProjectName)
  ) {
    return false;
  } else {
    return true;
...
function createTask(formData) {
  ...
  if (!Object.hasOwn(projects, forProject)) {
    throw new Error(
      `Can't create task for project ${forProject} because that project can't be found.`,
    );
  }
...
function updateTask(projectName, title, formData) {
  ...
  if (!Object.hasOwn(projects, projectName)) {
    throw new Error(
      `Can't update task for project ${formData.forProject} because that project can't be found.`,
    );
  }
  if (!projects[projectName].hasTask(title)) {
    throw new Error(
      `Can update task ${title} on ${projectName} because it doesn't exist.`,
    );
  }
...
function deleteTask(projectName, taskTitle) {
  if (!Object.hasOwn(projects, projectName)) {
    throw new Error(
      `Can't remove task from project ${projectName} as it can't be found`,
    );
  }
  const project = projects[projectName];
  if (!project.hasTask(taskTitle)) {
    throw new Error(
      `Can't delete task ${taskTitle} as their is no task by that name on this project.`,
    );
  }
